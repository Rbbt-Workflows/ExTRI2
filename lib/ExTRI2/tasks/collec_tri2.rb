...
module ExTRI2
  ...
  dep :ExTRI2_clean, compute: :produce, remove_auto_regulation: true do |jobname,options|
    ...
    when :complete
      {task: :ExTRI2_clean, inputs: options.merge(remove_auto_regulation: false), jobname: jobname}
    when :clean
      {task: :ExTRI2_clean, jobname: jobname}
    when :clean_no_MoR
  ...
  task_alias :CollecTRI2_pre, ExTRI, :CollecTRI, confidence: 'none' do |jobname,options,dependencies|
